...
class ComplexConv(nn.Module):
    def __init__(self, in_features: int, out_features: int, bias: bool = False, contiguous: bool = True):
        ...
        self.weight = nn.Parameter(
            torch.complex((1 / (in_features * out_features)) * torch.Tensor(out_features, in_features, 1),
...
class MWT_CZ1d_cross(nn.Module):
    def __init__(self, in_channels, out_channels, seq_len_q, seq_len_kv, modes1, c=64,
        ...
        self.attn1 = SpectralCross1d(in_channels=in_channels, out_channels=out_channels, seq_len_q=seq_len_q,
                                     seq_len_kv=seq_len_kv, modes1=modes1, activation=activation)
        self.attn2 = SpectralCross1d(in_channels=in_channels, out_channels=out_channels, seq_len_q=seq_len_q,
                                     seq_len_kv=seq_len_kv, modes1=modes1, activation=activation)
        self.attn3 = SpectralCross1d(in_channels=in_channels, out_channels=out_channels, seq_len_q=seq_len_q,
                                     seq_len_kv=seq_len_kv, modes1=modes1, activation=activation)
        self.attn4 = SpectralCross1d(in_channels=in_channels, out_channels=out_channels, seq_len_q=seq_len_q,
                                     seq_len_kv=seq_len_kv, modes1=modes1, activation=activation)
